...
$expireInDays='14'
$from='GSD@petrofac.com'
$logging='Enabled'
$logPath='C:\Users\admin_seneni\Desktop\Script-logs'
$interval ='14'
###################################################################################################################
# Time / Date Info
$start = [datetime]::Now
...
foreach ($user in $notifyUsers)
{
   ...
   $body ="
    ...
    <p>Your password will expire $messageDays You should change it now to avoid being locked out of your Petrofac email account.<br>
    
    <p><b>If you are on the Petrofac network,</b><br> 
    Hit <b> ctrl+alt+del</b>  on your keyboard, then select <b> change password</b> and follow the prompts.
    
    <p><b>If you are not on the Petrofac network</b><br>
    You can change your password through <a href=""https://outlook.office365.com/owa""> Office365</a> or http://myapps.microsoft.com. Click on the circle with your initials in the top right hand corner of the page, select <b> My account </b>, then <b> Security and Privacy </b>, then <b>Password</b> and follow the prompts to change your password. <br>
    
    <p>For more information, visit the <a href=""https://petrofaconline.sharepoint.com/sites/Petronet/SiteAssets/Info%20Sec/Self-service%20Password%20Reset%20user%20guide%20(final)%20Dec%202016.pdf"" > Self Service Password Reset </a> User Guide when you’re on the Petrofac network.<br>
 
    <p>Thanks,<br>
    </p>
    GSD
    <a href=""mailto:GSD@petrofac.com""?Subject=Password Expiry Assistance"">GSD@petrofac.com</a> 
    </font>"
    ...
    try{
        ...
        if($interval)
        {
            ...
            if($daystoExpire -eq '1')
            {
                ...
                Send-Mailmessage -smtpServer $smtpServer -Port 25 -from $from -to $emailaddress -subject $subject -body $body -bodyasHTML -priority High -Encoding $textEncoding -ErrorAction Stop
                $user | Add-Member -MemberType NoteProperty -Name SendMail -Value "OK" -Force
            }
            elseif($daystoExpire -eq '7')
            {
                # if using status - output information to console
                if($status)
                {
            ...
            else
            {
                ...
                $user | Add-Member -MemberType NoteProperty -Name SendMail -Value "Skipped - Interval" -Force
            }
        }
        else
        {
...
if($logging)
{
    ...
    if($reportTo)
    {
        ...
        try{
            Send-Mailmessage -smtpServer $smtpServer -Port 25 -from $from -to febin.frederic@petrofac.com -subject $reportSubject -body $reportbody -bodyasHTML -priority High -Encoding $textEncoding -Attachments $logFile -ErrorAction Stop 
        }
        catch{
            $errorMessage = $_.Exception.Message
...
$notifiedUsers | select UserName,Name,EmailAddress,PasswordSet,DaysToExpire,ExpiresOn | sort DaystoExpire |Export-Csv C:\Users\admin_febinf\Desktop\Script-logs\user.csv -NoTypeInformation
Send-MailMessage -SmtpServer $smtpServer -Port 25 -From GSD@petrofac.com -To febin.frederic@petrofac.com -Subject "Report-PWD Expiry userNotification" -Attachments C:\Users\admin_febinf\Desktop\Script-logs\user.csv 
